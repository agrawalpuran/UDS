...
export async function createProduct(productData: {
  ...
  console.log('[createProduct] Product data to create:', JSON.stringify(productDataToCreate, null, 2))
  const newProduct = await Uniform.create(productDataToCreate)
  console.log('[createProduct] Created product:', JSON.stringify(newProduct.toObject(), null, 2))
...
export async function getCompanyById(companyId: string | number): Promise<any | null> {
  ...
  // Find company by numeric ID first (since company.id is now numeric)
  // Explicitly select all fields including enableEmployeeOrder to ensure it's included
  let company = null
  if (numericCompanyId !== null) {
    company = await Company.findOne({ id: numericCompanyId })
      .select('id name logo website primaryColor secondaryColor showPrices allowPersonalPayments allowPersonalAddressDelivery enableEmployeeOrder adminId createdAt updatedAt')
      .populate('adminId', 'id employeeId firstName lastName email')
      .lean()
  }
...
export async function createEmployee(employeeData: {
  ...
  if (employeeData.locationId) {
    ...
    const locationCompanyId = typeof location.companyId === 'object' && location.companyId?.id
      ? location.companyId.id
      : (await Company.findById(location.companyId))?.id
    
    if (locationCompanyId !== employeeData.companyId) {
      throw new Error(`Location ${employeeData.locationId} does not belong to company ${employeeData.companyId}`)
    }
...
export async function updateEmployee(
  ...
  if (updateData.locationId !== undefined) {
    if (updateData.locationId) {
      ...
      const employeeCompanyId = typeof employee.companyId === 'object' && employee.companyId?.id
        ? employee.companyId.id
        : (await Company.findById(employee.companyId))?.id
      ...
      if (locationCompanyId !== employeeCompanyId) {
        throw new Error(`Location ${updateData.locationId} does not belong to employee's company`)
      }
...
export async function validateEmployeeEligibility(
  ...
  const { totalEligibility, cycleDurations } = await getEmployeeEligibilityFromDesignation(employeeId)
...
export async function createProductVendor(productId: string, vendorId: string): Promise<void> {
  ...
  // Auto-create VendorInventory record if it doesn't exist
  try {
    await ensureVendorInventoryExists(vendor._id, product._id)
  } catch (inventoryError: any) {
    // Log but don't fail the product-vendor linking if inventory creation fails
    console.error('createProductVendor - Warning: Failed to create inventory record:', inventoryError.message)
    // Continue - inventory can be created later via backfill or manual update
  }
}
  ...
  for (const productId of productIds) {
    try {
      ...
      try {
        ...
        // Log but don't fail the product-vendor linking if inventory creation fails
        console.error(`createProductVendorBatch - Warning: Failed to create inventory for product ${productId}:`, inventoryError.message)
        // Continue - inventory can be created later via backfill or manual update
      }
...
export async function getVendorInventory(vendorId: string, productId?: string): Promise<any[]> {
  ...
  const rawInventoryRecords = await db.collection('vendorinventories').find(query).toArray()
  ...
  if (inventoryRecords.length > 0) {
    const db = mongoose.connection.db
    if (db) {
      const rawInventory = await db.collection('vendorinventories').find(query).toArray()
...
/**
 * Helper function to ensure a VendorInventory record exists for a vendor-product pair
 * This is called automatically when products are linked to vendors
 * Idempotent: safe to call multiple times
 */
async function ensureVendorInventoryExists(vendorId: mongoose.Types.ObjectId, productId: mongoose.Types.ObjectId): Promise<void> {
  try {
    // CRITICAL: Verify product exists before creating inventory
    const product = await Uniform.findById(productId)
    if (!product) {
      console.error(`❌ Cannot create VendorInventory: Product with ObjectId ${productId} does not exist in Uniform collection`)
    ...
    if (existingInventory) {
      // Inventory already exists, no need to create
      return
    }
    ...
    console.log(`✅ Created VendorInventory for vendor ${vendorId} / product ${productId} (product: ${product.id || productId})`)
  } catch (error: any) {
    // If error is due to duplicate (race condition), that's okay
    if (error.code === 11000 || error.message?.includes('duplicate')) {
      console.log(`⚠️  VendorInventory already exists for vendor ${vendorId} / product ${productId} (race condition)`)
      return
    }
    // Re-throw other errors (including product not found)
    console.error(`❌ Error ensuring VendorInventory exists:`, error)
    throw error
  }
}
...
export async function updateVendorInventory(
  ...
  if (lowInventoryThreshold !== undefined) {
    thresholdMap = new Map(Object.entries(lowInventoryThreshold))
  ...
  const thresholdObj = inventory.lowInventoryThreshold instanceof Map
    ? Object.fromEntries(inventory.lowInventoryThreshold)
    : inventory.lowInventoryThreshold || {}
