  ...
  if (fs.existsSync(dbDir)) {
    ...
    if (hasBackup) {
      ...
      if (fs.existsSync(metadataFile)) {
        ...
        metadata.collections.forEach((col: any) => {
...
1. **Application Code** (`/application`)
...
2. **Database Backup** (`/database`)
   - Full MongoDB database dump
   - All collections with data
   - Indexes preserved
   - Backup method: ${dbInfo.method || 'mongodump'}
...
async function main() {
  ...
  try {
    ...
    // Backup database
    const dbInfo = await backupDatabase(backupDir)
