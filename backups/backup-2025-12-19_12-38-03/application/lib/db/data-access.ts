...
export async function getCompanyById(companyId: string | number): Promise<any | null> {
  ...
  // Find company by numeric ID first (since company.id is now numeric)
  // Explicitly select all fields including enableEmployeeOrder and allowLocationAdminViewFeedback to ensure they're included
  let company = null
  if (numericCompanyId !== null) {
    company = await Company.findOne({ id: numericCompanyId })
      .select('id name logo website primaryColor secondaryColor showPrices allowPersonalPayments allowPersonalAddressDelivery enableEmployeeOrder allowLocationAdminViewFeedback adminId createdAt updatedAt')
      .populate('adminId', 'id employeeId firstName lastName email')
      .lean()
  }
  ...
  if (updateData.locationId !== undefined) {
    if (updateData.locationId) {
      ...
      if (!employeeCompanyId || !locationCompanyId) {
        console.warn(`[updateEmployee] ⚠️ Could not determine company IDs:`, {
          employeeCompanyId: employeeCompanyId || 'null',
          locationCompanyId: locationCompanyId || 'null'
        })
      }
      
      if (locationCompanyId !== employeeCompanyId) {
        throw new Error(`Location ${updateData.locationId} does not belong to employee's company. Employee company: ${employeeCompanyId || 'unknown'}, Location company: ${locationCompanyId || 'unknown'}`)
      }
