...
import { getEmployeeByEmail, getOrdersByEmployee, getCompanyById, getVendorByEmail, createProductFeedback, getProductFeedback, getProductById } from '@/lib/data-mongodb'
...
export default function ConsumerOrdersPage() {
  ...
  const handleSubmitReturnRequest = async () => {
    ...
    try {
      const userEmail = localStorage.getItem('userEmail') || currentEmployee.email
                    ...
                    {order.items && Array.isArray(order.items) && order.items.length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {order.items.map((item: any, idx: number) => {
                          ...
                          // For split orders, check the item's specific status (_itemStatus)
                          // For regular orders, use the order's status
                          const itemStatus = (item as any)._itemStatus || order.status
                              ...
                              {order.status === 'Delivered' ? (
                                  ...
                                  {existing ? (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-2">
                                      <div className="flex items-center space-x-1 mb-2">
                                        ...
                                        <span className="text-xs font-semibold text-green-900">Feedback Submitted</span>
                                      </div>
                                      <div className="flex items-center space-x-1 mb-2">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                          <Star
                                        ...
                                        <span className="text-xs text-gray-600 ml-1">({existing.rating}/5)</span>
                                      </div>
                                      {existing.comment && (
                                        <p className="text-xs text-gray-700 line-clamp-2">{existing.comment}</p>
                                      )}
                                    </div>
                                  ) : isSubmitting ? (
                                ...
                                <div className="flex-1 flex items-center justify-center">
                                  <div className="text-center">
                                    <p className="text-xs text-gray-500 font-medium">{order.status || 'Processing'}</p>
