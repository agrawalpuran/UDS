...
export async function updateOrderStatus(orderId: string, status: 'Awaiting approval' | 'Awaiting fulfilment' | 'Dispatched' | 'Delivered'): Promise<any> {
  ...
  // If this is a replacement order being delivered, complete the return request and increment inventory for returned item
  if (status === 'Delivered' && (order as any).orderType === 'REPLACEMENT' && (order as any).returnRequestId) {
    try {
      console.log(`[updateOrderStatus] 🔄 Replacement order delivered, processing return request: ${(order as any).returnRequestId}`)
      ...
      if (returnRequest && returnRequest.status === 'APPROVED') {
        ...
        // Increment inventory for the returned item (original size)
        if (returnRequest.uniformId && returnRequest.originalSize && returnRequest.requestedQty) {
          try {
            console.log(`[updateOrderStatus] 📦 Incrementing inventory for returned item:`, {
            ...
            const replacementOrderVendorId = order.vendorId
            if (!replacementOrderVendorId) {
              ...
              if (replacementOrderVendorId instanceof mongoose.Types.ObjectId) {
                vendorObjectId = replacementOrderVendorId
              } else {
              ...
              try {
                ...
                if (!inventory) {
                  console.warn(`[updateOrderStatus] ⚠️ No inventory record found for vendor ${vendorObjectId} and product ${product.id}, creating one`)
            ...
            console.error(`[updateOrderStatus] ⚠️ Failed to increment inventory for returned item: ${error.message}`)
            // Don't throw - order status update should succeed even if inventory increment fails
          }
        }
      }
    } catch (error: any) {
      console.error(`[updateOrderStatus] ⚠️ Failed to complete return request: ${error.message}`)
      // Don't throw - order status update should succeed even if return completion fails
    }
  }
...
export async function createReturnRequest(requestData: {
  ...
  const orderItem = order.items[requestData.originalOrderItemIndex]
  
  // Get employee - use getEmployeeByEmail for robust lookup (handles encrypted emails)
  // Since we're in the same file, we can call it directly
  let employee = await getEmployeeByEmail(requestData.requestedBy)
  
  // If not found by email, try by employeeId or id
  if (!employee) {
  ...
  // Get company ID
  const companyId = typeof order.companyId === 'object' && order.companyId?._id
    ? order.companyId._id
    : order.companyId
  
  // Get employee ID
  const employeeId = typeof order.employeeId === 'object' && order.employeeId?._id
    ? order.employeeId._id
    : order.employeeId
...
export async function approveReturnRequest(
  ...
  // Update replacement orders to mark them as REPLACEMENT type and auto-approve
