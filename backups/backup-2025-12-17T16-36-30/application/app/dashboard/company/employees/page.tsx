...
export default function EmployeesPage() {
      ...
      const loadData = async () => {
        try {
          ...
          if (targetCompanyId) {
              ...
              companyLocations.forEach((loc: any) => {
                if (loc.adminId) {
                  ...
                  if (typeof loc.adminId === 'object') {
                    ...
                    // SECONDARY: Also track employeeId (numeric string like '300032') as fallback
            ...
            console.log('[EmployeesPage] Total unique Location Admin IDs:', adminIds.size, Array.from(adminIds))
  ...
  const handleEditEmployee = (employee: any) => {
    ...
    setFormData({
      ...
      locationId: employee.locationId?._id?.toString() || employee.locationId?.id || employee.locationId || '',
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mb-4 flex items-center justify-between text-xs">
            <div className="flex items-center space-x-3 flex-1">
              ...
              <div className="flex items-center space-x-4 text-blue-800">
                ...
                {locationAdmins.size > 0 && (
                  <span className="text-blue-900 font-semibold">
                    ({locationAdmins.size} found)
                  </span>
                )}
              </div>
            </div>
                              ...
                              >
                                ...
                                <td className="py-4 px-6 text-gray-900 font-medium whitespace-nowrap">
                                  <div className="flex items-center space-x-2">
                                    <span>{masked.firstName} {masked.lastName}</span>
                ...
                <div>
                  ...
                  >
                    ...
                    {locations.map((location) => (
                      <option key={location._id?.toString() || location.id} value={location._id?.toString() || location.id}>
                        {location.name}
                      </option>
                    ))}
                  </select>
