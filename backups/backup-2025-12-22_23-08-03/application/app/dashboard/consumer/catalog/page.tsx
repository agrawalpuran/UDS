...
import { getProductsByCompany, getEmployeeByEmail, getConsumedEligibility, getCompanyById, isCompanyAdmin, getLocationByAdminEmail, getBranchByAdminEmail, getVendorByEmail, Uniform } from '@/lib/data-mongodb'
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (vendor) {
          console.error('Consumer Catalog - Email belongs to vendor, not employee:', userEmail)
          console.error('Consumer Catalog - Redirecting to vendor portal...')
          alert(`This email (${userEmail}) is registered as a vendor. Please access the vendor portal instead.`)
        ...
        // Load products (filtered by designation and gender), consumed eligibility, total eligibility, and company settings in parallel
        const [products, consumed, companyData, eligibilityResponse] = await Promise.all([
