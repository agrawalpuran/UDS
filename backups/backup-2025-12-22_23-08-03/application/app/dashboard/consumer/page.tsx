...
import { getProductsByCompany, getEmployeeByEmail, getOrdersByEmployee, getConsumedEligibility, getCompanyById, isCompanyAdmin, getLocationByAdminEmail, getBranchByAdminEmail, getVendorByEmail, Uniform } from '@/lib/data-mongodb'
...
export default function ConsumerDashboard() {
  ...
  const [totalEligibility, setTotalEligibility] = useState<{
    shirt: number
    pant: number
    shoe: number
    jacket: number
    ...
    if (typeof window !== 'undefined') {
      const loadData = async () => {
        try {
          ...
          try {
            // ROLE DETECTION: Check if email belongs to vendor, company admin, or employee
            // This prevents vendor emails from being treated as employees
            const [vendor, employee] = await Promise.all([
              getVendorByEmail(userEmail),
              getEmployeeByEmail(userEmail)
            ])
            
            // If email belongs to a vendor, redirect to vendor portal
            if (vendor) {
              console.error('Consumer Dashboard - Email belongs to vendor, not employee:', userEmail)
              console.error('Consumer Dashboard - Redirecting to vendor portal...')
              setError(`This email (${userEmail}) is registered as a vendor. Please access the vendor portal instead.`)
              setTimeout(() => {
            ...
            // Load products (filtered by designation and gender), orders, consumed eligibility, total eligibility, and company settings in parallel
            ...
            if (eligibilityResponse) {
              setTotalEligibility({
                shirt: eligibilityResponse.shirt || 0,
                pant: eligibilityResponse.pant || 0,
                shoe: eligibilityResponse.shoe || 0,
                jacket: eligibilityResponse.jacket || 0,
              })
            } else {
  ...
  const getEligibilityForCategory = (category: string): number => {
    ...
    // Use designation-level eligibility (from API) instead of employee-level eligibility
    // This ensures we always use the latest eligibility rules from designation
    const totalEligibilityValue = (() => {
      switch (category) {
        case 'shirt': return totalEligibility.shirt
        case 'pant': return totalEligibility.pant
        case 'shoe': return totalEligibility.shoe
        case 'jacket': return totalEligibility.jacket
        default: return 0
      }
    })()
      ...
      <div>
          ...
          <div className="enterprise-card mb-8">
            <div className="enterprise-card-body">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div>
                  <span className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Company</span>
                  <p className="text-sm font-medium text-neutral-900 mt-1.5">{userProfile?.company?.name || company?.name || '-'}</p>
                </div>
                <div>
                  <span className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Role</span>
                  <p className="text-sm font-medium text-neutral-900 mt-1.5">
                    {userProfile?.role 
                ...
                {userProfile?.role === 'LOCATION_ADMIN' && userProfile?.location && (
                  <div>
                    ...
                    <p className="text-sm font-medium text-neutral-900 mt-1.5">{userProfile.location.name || '-'}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            ...
            const StatCard = (
              <div 
                className={`enterprise-card p-6 ${isClickable ? 'cursor-pointer' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <p className="text-xs font-medium text-neutral-500 uppercase tracking-wide mb-2">{stat.name}</p>
        ...
        <div className="enterprise-card mb-8">
          <div className="enterprise-card-header">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-neutral-900">Order New Uniforms</h2>
                <p className="text-sm text-neutral-500 mt-1">Quickly add items to your cart and checkout</p>
              </div>
              <Link 
                href="/dashboard/consumer/catalog" 
                style={{ color: company?.primaryColor || '#032D42' }}
                className="text-sm font-medium text-neutral-700 hover:text-neutral-900 flex items-center space-x-1 transition-colors"
              >
                <span>View All</span>
          ...
          {quickOrderProducts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-neutral-600">No products available for your company.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              {quickOrderProducts.map((uniform) => {
                ...
                return (
                  <div key={uniform.id} className="enterprise-card p-4">
                    <div className="relative h-32 bg-white rounded-lg mb-3 overflow-hidden">
                      <Image
                        src={getIndigoUniformImage(uniform.category, uniform.gender, uniform.name)}
                        alt={uniform.name}
                        fill
