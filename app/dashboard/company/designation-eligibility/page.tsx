...
export default function DesignationEligibilityPage() {
      ...
      const loadData = async () => {
        try {
          ...
          if (storedCompanyId) {
            ...
            // This ensures all categories are available for designation eligibility configuration
  ...
  const getFilteredCategoriesForGender = (gender: 'male' | 'female' | 'unisex') => {
    if (gender === 'unisex') {
      return availableCategories
    }
    
    // For specific genders, show all categories that have products in that category
    // The gender filtering will happen when products are displayed, not when categories are shown
    // This allows admins to set up eligibility rules even if products of that gender don't exist yet
    ...
    const filtered = availableCategories.filter(category => {
      ...
      // Show category if it has any products (regardless of gender)
      // The gender filter will be applied when products are actually displayed to employees
  ...
  const handleEdit = (eligibility: any) => {
    ...
    // Ensure all selected categories have itemEligibility data
    for (const categoryId of categories) {
      ...
      if (elig && typeof elig === 'object') {
        // Use existing data - preserve actual values, don't default
        ...
        itemElig[categoryId] = {
          quantity: qty > 0 ? qty : 1, // Only default to 1 if value is 0 or invalid
          renewalFrequency: freq > 0 ? freq : 6, // Only default to 6 if value is 0 or invalid
          renewalUnit: unit,
        }
        
        console.log(`  ✅ Loaded ${categoryId} from DB:`, itemElig[categoryId])
      } else {
    ...
    console.log('🔍 Final itemEligibility for form:', JSON.stringify(itemElig, null, 2))
    
    setFormData({
      designation: eligibility.designation,
      gender: eligibility.gender || 'unisex',
      allowedProductCategories: categories,
      itemEligibility: itemElig,
    })
    setShowAddForm(false)
  }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      for (const categoryId of formData.allowedProductCategories) {
        ...
        console.log(`Building itemEligibility for update - ${categoryId}:`, {
          raw: itemElig,
          quantity: itemElig?.quantity,
          renewalFrequency: itemElig?.renewalFrequency,
          renewalUnit: itemElig?.renewalUnit,
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                const newGender = e.target.value as 'male' | 'female' | 'unisex'
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select which gender this eligibility rule applies to. "Unisex" applies to both male and female employees. Available categories will update based on your selection.
                </p>
              </div>
                  ...
                  {eligibilities.map((eligibility) => (
                    <tr key={eligibility.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-4 px-6">
                        ...
                        >
                          {eligibility.gender === 'unisex' || !eligibility.gender ? 'Unisex' : eligibility.gender}
                        </span>
                      </td>
                      <td className="py-4 px-6">
