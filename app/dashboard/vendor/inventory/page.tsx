...
import { getProductsByVendor, getVendorById } from '@/lib/data-mongodb'
    ...
    const loadData = async () => {
      try {
        ...
        if (storedVendorId) {
          ...
          setInventory(vendorProducts)
        } else {
          // Default to first vendor for demo
          setVendorId('VEND-001')
          const vendorProducts = await getProductsByVendor('VEND-001')
          setInventory(vendorProducts)
        }
      } catch (error) {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredInventory.map((item: any) => (
            <div key={item.id} className="bg-white rounded-xl shadow-lg overflow-hidden">
              <div className="relative h-48 bg-gradient-to-br from-indigo-50 via-blue-50 to-slate-100">
                <Image
                  src={getIndigoUniformImage(item.category, item.gender)}
                  alt={item.name}
                  fill
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    item.stock > 50 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                  }`}>
                    Stock: {item.stock}
                  </span>
                </div>
                <div className="flex space-x-2">
                  <button className="flex-1 bg-blue-600 text-white py-2 rounded-lg font-semibold hover:bg-blue-700 transition-colors text-sm flex items-center justify-center space-x-1">
